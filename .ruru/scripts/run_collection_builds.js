...
function main() {
    ...
    // 1. Resolve Core Modes first (moved to inside the loop)
    ...
    for (const collectionName of collectionNames) {
        console.log(`\nProcessing collection: ${collectionName}`);
         let coreModes = []; // Reset coreModes for each collection
        if (collections.core) {
        ...
        if (finalModeList.length === 0 && !isSpecialFullCollection) {
             ...
             continue;
                ...
                if (!isSpecialFullCollection) {
                    ...
                    roomodesWritten = deleteRoomodes();
                    if (!roomodesWritten) {
                        console.error(`Failed to delete .roomodes for collection '${collectionName}'. Skipping build.`);
                        allBuildsSuccessful = false;
                        continue;
                    }
                }
                ...
                if (!isSpecialFullCollection) {
                    const roomodesDeleted = deleteRoomodes();
                     if (!roomodesDeleted) {
                         console.error(`Failed to delete .roomodes for collection '${collectionName}'. Continuing anyway.`);
                     }
                }
            }
